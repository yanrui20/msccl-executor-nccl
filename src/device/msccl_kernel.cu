...
__device__ __forceinline__ void pcclRunInterpreter(struct ncclDevComm* comm, struct mscclAlgo* algo, struct mscclWork work) {
...
#define PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(type) \
__global__ void PCCL_KERNEL_ENTRY_NAME(type)(struct ncclDevComm* comm, struct mscclAlgo* algo, struct mscclWork work) { \
  pcclRunInterpreter<type, FuncSum<type>, ProtoSimple<MSCCL_CHUNKSTEPS/MSCCL_SLICESTEPS, MSCCL_SLICESTEPS>>(comm, algo, work); \
}

#if defined(__CUDA_BF16_TYPES_EXIST__) && defined(__CUDA_FP8_TYPES_EXIST__)
#define PCCL_IMPL_KERNEL_ENTRY_FUNC() \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(int8_t) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(uint8_t) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(int32_t) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(uint32_t) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(int64_t) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(uint64_t) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(half) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(float) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(double) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(__nv_bfloat16) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(__nv_fp8_e4m3) \
  PCCL_IMPL_KERNEL_ENTRY_FUNC_TYPE(__nv_fp8_e5m2)
